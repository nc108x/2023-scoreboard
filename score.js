...
function pole_button_listener(event, color, pole_no, pole_html) {
    ...
    pole_html.style.background = color == "red" ? "#f5c2e7" : "#89b4fa";
    console.log(pole_no);
...
function gamefield_init() {
    ...
    for (let i = 0; i < pole_buttons.length; i++) {
        ...
        pole_buttons[i].addEventListener("click", (event) => {
            pole_button_listener(event, "red", i, pole_buttons[i]);
        });
        pole_buttons[i].addEventListener("contextmenu", (event) => {
            pole_button_listener(event, "blue", i, pole_buttons[i]);
        });
    }
}
